...
export abstract class TabSingleSegmentComponent extends TabBaseComponent {
  ...
  constructor(
    ...
    protected cronBootstrap: CronBootstrapService
  ) {
    super();
  }
  ...
  inSpecificsList(value: string, mode: Mode) {
    return this.cronBootstrap.hasValue(value, this.segment, mode);
  }

  toggleSpecifics(value: string, mode: Mode) {
    const values = this.cronBootstrap.getValues(this.segment, mode);
