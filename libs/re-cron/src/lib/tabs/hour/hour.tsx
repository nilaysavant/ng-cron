...
export class ReCronHour extends TabSingleSegmentComponent<ReCronHourState> {
	constructor(props: TabBaseProps) {
		super(Segment.hours, props);

		this.state = {
			hourCodes: new CoreService().getList(Segment.hours, true),
			hours: new CoreService().getList(Segment.hours),
			view: QuartzCronDI.get(this.props.session).getView(Segment.hours)
		};
	}
